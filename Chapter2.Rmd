...
storm2012 <- read_csv("stormdata_2012.csv")
storm2013 <- read_csv("stormdata_2013.csv")
...
states_sandyTwi <- c("Maryland", "Delaware", "New Jersey", "New York", "Connecticut", 
                    "Massachusetts", "Rhode Island", "North Carolina", "Virginia", 
                    "West Virginia", "Ohio", "Pennsylvania", "New Hampshire")
...
sandy_states <- sandyTweets %>% 
  select(lat = latitude, 
         ...
         state = c_state,
         tweet = text) %>% 
  filter(state %in% states_sandyTwi) %>%
