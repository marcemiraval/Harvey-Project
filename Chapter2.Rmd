...
sandySpot_sf <- st_as_sf(sandySpot, coords = c("BEGIN_LAT","BEGIN_LON"))
...
```{r}
#sandySpot_sf$hex <- over(sandySpot_sf, sp_hex)$id # Previous Method
sf_hex$group <- 1:nrow(sf_hex)

# Funciona
test1 <- st_join(sandySpot_sf, sf_hex) %>% #st_intersects is by the function by default here.
  mutate(id = strtrim(row.names(.), 1)) %>% # Pero esto no lo entiendo debe haber dentro de las funciones una que me permita traer el atributo que quiero del hexagono. Creo que la cosa está entre dpylr y sf. Ver aquí: https://github.com/r-spatial/sf/issues/429
  group_by(id)

test2 <- st_join(sf_hex, sandySpot_sf)
```
