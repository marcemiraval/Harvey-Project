...
```{r}
min(hexWithTweets$total)
max(hexWithTweets$totRatio)
qplot(total, data = hexWithTweets, geom = "bar", binwidth = 500)
qplot(totRatio, data = hexWithTweets, geom = "bar", binwidth =0.005)
qplot(total, data = hexWithSpoReports, geom = "bar")
qplot(totRatio, data = hexWithSpoReports, geom = "bar")


p <- ggplot(data = hexWithTweets, aes(total, group)) + geom_count()
p
q <- ggplot(data = hexWithSpoReports, aes(total, group)) + geom_count(aes(size = ..prop..))
q
```
