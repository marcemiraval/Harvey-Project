---
title: "Colorado Flood Risk Areas During Flood"
author: "Marcela"
date: "June 22, 2020"
...
colo_Flood <- read_csv("Data/colo_Flood_IA.csv") %>% 
...
tweetInFlood <- st_join(colo_Flood, floodExtent) %>% 
  filter(!is.na(Id))
...
k_result_tweetInFlood %>%
  ...
  filter(K %in% c(7,8,14)) %>% # This values should be the same used when creating the model in the R script
  unnest() %>%
...
```{r beta, echo=FALSE, message=FALSE, fig.width=13, fig.height=7}
...
heat_floodedArea <- td_gamma_floodedArea %>% 
  ...
  left_join(colo_Flood, by = "tweet_id") %>% 
...
colo_Flood <- colo_Flood %>% 
  mutate(lon = st_coordinates(.)[,1]) %>% 
  mutate(lat = st_coordinates(.)[,2])

tweet_and_topic <- dplyr::left_join(heat_floodedArea, colo_Flood, by = "tweet_id") %>%
...
StructurePoints <- st_read("Data/GroundTruthData/Colorado_Flooding_Structures_26Sep2013_FINAL/CO_Flooding_Structures_Final_edits_26SEP13.shp")

tmap_mode("view") 
map <- tm_shape(damagePols) + 
  tm_polygons(col = "#f4a582",
              alpha = 0.8) +
  tm_shape(StructurePoints) + 
  tm_dots(col = "#ca0020",
             scale=0.8,
             alpha = 1)
map + tm_basemap(server = "Stamen.TonerLite")
