...
keywords <- c("boulder", "boulderflood", "cowx", "amp", "|", "rt", "coflood")
...
saveRDS(flood_county_tweet_sparse, file = "Data/flood_county_tweet_sparse.rds")
```
...
k_result_flood_county <- readRDS(file = "Data/k_result_flood_county.rds")
...
k_result_flood_county %>%
  ...
  filter(K %in% c(6,9,14,17)) %>% # This values should be the same used when creating the model in the R script
  unnest() %>%
...
```{r gamma, echo=FALSE, message=FALSE, warning=FALSE, width = 0.9}
library(ggthemes)
library(extrafont)
