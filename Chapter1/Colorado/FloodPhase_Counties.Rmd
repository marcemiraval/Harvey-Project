...
**4086 tweets** sent during the disaster were filtered based on the 9 counties affected in the disaster --according to the reports: Arapahoe, Boulder, Denver, El Paso, Jefferson, Larimer, Logan, Morgan and Weld.
...
flood_county_tweet <- st_join(colo_sf_p, affected_counties_p) %>% 
...
From the **2804 tweets** resulting from this process, only **849** tweets sent during the "Flood" stage were filtered for content analysis.
...
saveRDS(flood_county_tweet_sparse, file = "flood_county_tweet_sparse.rds")
```
...
k_result_flood_county <- readRDS(file = "k_result_flood_county.rds")
...
k_result_flood_county %>%
  ...
  labs(x = "K (number of topics)",
       ...
       subtitle = "These diagnostics indicate that a good number of topics would be around 14")
```
...
k_result_flood_county %>%
  ...
  filter(K %in% c(4, 9, 14)) %>% # This values should be the same used when creating the model in the R script
  unnest() %>%
...
tweet_and_topic_geo <- tweet_and_topic %>% 
  st_as_sf(coords = c("lon", "lat"), crs = 4326) %>% 
  filter(topic == "Topic 4" | topic == "Topic 2")
```
