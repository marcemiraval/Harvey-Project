...
keywords <- c("boulder", "boulderflood", "coflood", "cofloodrelief", "colorado")
colo_tidy <- colo_clean %>% 
  mutate(tweet = text) %>% # To keep a column with the original tweet
  unnest_tokens(word, text, token = "tweets") %>% # add a row for each token (word) and repeat the other information
  anti_join(stop_words) %>%  # remove stopwords
...
td_beta %>% 
  ...
  top_n(10) %>% 
  ungroup %>% 
  mutate(term = reorder(term, beta)) %>% #reordering by tf_idf
  ggplot(aes(term, beta, fill = topic)) +
  geom_col(alpha = 0.8, show.legend = FALSE) +
  facet_wrap(~topic, scales = "free") +
  coord_flip() # To see which words contribute the most to each topic
```
