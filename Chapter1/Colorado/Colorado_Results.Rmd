...
```{r tweets distribution, echo=FALSE, message=FALSE}
library(scales)
...
sep15_count <- colo_days %>% 
  filter(day == 15) %>% 
  nrow()
```

Notice that there were only 5 observations on September 15 and only 131 on September 15.
...
topic_model <- stm(colo_dfm, K = 6, init.type = "Spectral") # unsupervised 
summary(topic_model)
```
...
heat <- td_gamma %>% 
  ...
  left_join(colo_days, by = "text") %>% 
  select(text, topic, gamma, stage, day)
```

```{r gamma_plot, echo=FALSE, message=FALSE, eval = FALSE}

ggplot(td_gamma, aes(gamma, fill = as.factor(topic))) +
  geom_histogram(alpha = 0.8, show.legend = FALSE) +
  facet_wrap(~ topic, ncol = 2) +
  labs(title = "Distribution of tweet probabilities for each topic",
       subtitle = "Each topic is associated with...",
       y = "Number of tweets", x = expression(gamma))


# x- axis values correspond to the estimated proportion of words from tweets that are generated from each topic.
# From the plot above, we can see that more probably each tweet is associated with a single topic. Gamma probabilities tell us which topics are coming from which documents (tweets). I don't think that at the end this plot is very meaningful.

```

```{r gamma_heatplot, echo=FALSE, message=FALSE}

library(RColorBrewer)
ggplot(heat, aes(day, topic )) +
  geom_tile(aes(fill = gamma), color = "white") +
  scale_colour_brewer(palette = "Greens", name = "Gamma") +
  ylab("Topic") +
  xlab("Flood day") +
...
Now I will repeat the same analysis but considering tweets sent from what should be the "affected area" (by using spatial clustering and by subseting by affected counties).
...
topic_model_clusters <- stm(clusters_dfm, K = 9, init.type = "Spectral") # unsupervised 
...
```{r gammaplot_clusters, echo=FALSE, message=FALSE}
...
```{r gammaclusters_heatplot, echo=FALSE, message=FALSE}

library(RColorBrewer)
ggplot(heat_clusters, aes(day, topic )) +
  geom_tile(aes(fill = gamma), color = "white") +
  scale_colour_brewer(palette = "Greens", name = "Gamma") +
  ylab("Topic") +
  xlab("Flood day") +
...
####  **Subsetting by Affected Counties**
...
#keywords_clusters <- c("boulder", "boulderflood", "colorado", "cowx", "flood", "coflood", "amp", "$")
keywords_clusters <- c("")
...
topic_model_tweet_in_county <- stm(tweet_in_county_dfm, K = 12, init.type = "Spectral") # unsupervised 
...
heat_tweet_in_county <- td_gamma_tweet_in_county %>% 
  rename(text = document) %>% 
  left_join(colo_days, by = "text") %>% 
  select(text, topic, gamma, day)
```

```{r gammaplot_clusters2, echo=FALSE, message=FALSE}
...
heat_county <- heat_tweet_in_county %>% 
  group_by(text) %>% 
