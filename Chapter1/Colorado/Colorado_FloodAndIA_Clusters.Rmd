...
```{r plotting_distance2, echo=FALSE, message=FALSE, fig.width=10, fig.height=15}

pts.wit.dist %>% 
  ggplot(aes(distancee, distClass, color = topic)) + # 
  geom_point(size = 2, alpha = 0.7) +
  labs(x = "Distance",
       y = "Date",
       title = "Distance between tweets and nearest reported structure",
...
```{r heatplot, echo=FALSE, message=FALSE}
#Assign color variables
col1 = "#0571b0" 
col2 = "#ca0020"
...
library(RColorBrewer)
ggplot(scaled_distancia, aes(topic, distClass)) +
  geom_tile(aes(fill = scaled_total_dist_topic), color = "white") +
  scale_fill_gradient(low = col1, high = col2) +  
  ylab("Distance Band") +
  xlab("Topic") +
  theme(legend.title = element_text(size = 10),
...
```{r plotting_distance, echo=FALSE, message=FALSE, fig.width=14, fig.height=10}

scaled_distancia %>% 
  ggplot(aes(date, scaled_total_dist_topic, color = topic)) + # 
  geom_point(size = 2, alpha = 0.7) +
  labs(x = "Date",
       y = "Distance",
       title = "Distance between tweets and nearest reported structure",
