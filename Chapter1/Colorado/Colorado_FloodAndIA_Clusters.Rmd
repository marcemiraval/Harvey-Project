...
write_csv(colo_clean, path = "colo_clean.csv")
```
...
write_csv(colo_peak_CO, path = "colo_Flood_IA.csv")
```
...
saveRDS(clusters_floodAndIA_sparse, file = "clusters_floodAndIA_sparse.rds")
saveRDS(clusters_floodAndIA_dfm, file = "clusters_floodAndIA_dfm.rds")
...
k_result_floodAndIA_cluster <- readRDS(file = "k_result_floodAndIA_clusters.rds") # This file is in the UCSB box folder since it's too heavy for github
...
htmlwidgets::saveWidget(TopicalMap, file = "MapTopicsFloodAndIAClusters.html")

```

```{r reading_floodextent_data, echo=FALSE, message=FALSE}

# From City of Boulder Website
CityOfBoulderFloods <- st_read(
  "GroundTruthData/Flood2013Extents/Flood2013Extents.shp")

# From Boulder County Website
BoulderCounty <- st_read(
  "GroundTruthData/Flood_2013_Inundated_Areas/Flood_2013_Inundated_Areas.shp")

# The following ones come from the disaster’s geoplatform’s website
CityOfBoulderFloods2 <- st_read(
  "GroundTruthData/Partial_Preliminary_2013_Colorado_Flooding_Extents/CityofBoulder.shp") #It's the same as the one from the other data source so I will skip this one

Evans <- st_read(
  "GroundTruthData/Partial_Preliminary_2013_Colorado_Flooding_Extents/Evans_SouthPlatte_Evans.shp")
```
