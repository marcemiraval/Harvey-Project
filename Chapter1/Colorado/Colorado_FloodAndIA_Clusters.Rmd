...
#setwd(dir= "Colorado") # Run in console
library(tidyverse)
...
clusters_df <- colo_clusters %>% 
  filter(cluster == 7 | cluster == 8 | cluster == 9 | cluster == 10 | cluster == 11 | cluster == 12) %>% 
...
keywords <- c("boulder", "boulderflood", "cowx", "coflood","colorado", "flood") #, "cowx", "$", "amp", "|", "rt", "coflood" Maybe?
...
k_result_floodAndIA_cluster %>%
  ...
  filter(K %in% c(4,7,10,12)) %>% # This values should be the same used when creating the model in the R script
  unnest() %>%
...
TopicalMap <- leaflet() %>% # Interactive map to see resulting clusters
  addTiles()  %>%
  addProviderTiles(providers$CartoDB.DarkMatter) %>% 
  addPolygons(data = damagePols, 
              color = "red",
              fillOpacity = 0.5,
              weight = 1.2) %>% 
  addCircles(data = tweet_and_topic_geo,
             weight = 3, 
             radius=20,
...
#htmlwidgets::saveWidget(TopicalMap, file = "Outputs/htmls_generated/MapTopicsFloodAndIAClusters.html")
...
original_structures <- st_read("Data/GroundTruthData/Colorado_Flooding_Structures_26Sep2013_FINAL/CO_Flooding_Structures_Final_edits_26SEP13.shp")
roads_affected <- st_read("Data/GroundTruthData/Colorado_Flooding_Roads_19Sep2013/Colorado_Flooding_Roads_19Sep2013.shp")
...
map <- tm_shape(damagePols) + 
  tm_polygons(col = "#756bb1",
              alpha = 0.8) +
  tm_shape(roads_affected) +
  tm_lines(col = "red") +
  tm_shape(original_structures) + 
  tm_dots(col = "red",
          scale=0.1,
          alpha = 1) +
...
#tmap_save(map, file = "Colorado/Outputs/htmls_generated/map_topics_FloodAndIA_Clusters.html")
htmlwidgets::saveWidget(map, file = "Colorado/Outputs/htmls_generated/map_topics_FloodAndIA_Clusters.html")
```
