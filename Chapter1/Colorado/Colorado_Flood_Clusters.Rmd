...
**4086 tweets** sent during the disaster were filtered based on spatial clusters
...
In order to identify tweets sent from the afected area, a hierarchical implementation of dbscan was used **(hdscan)**, considering a minimum of 315 tweets per cluster.
...
k_result_flood_cluster %>%
  ...
  filter(K %in% c(4,13,17)) %>% # This values should be the same used when creating the model in the R script
  unnest() %>%
...
<<<<<<< HEAD
<<<<<<< HEAD
```{r combine_datasets, echo=FALSE, message=FALSE}

# Combine datasets to get tweets and location
=======
```{r}
>>>>>>> f196eb6f299608ac4f356d51d652fe65d2010fa6
=======


```{r joinTweetsTopics, echo=FALSE, message=FALSE}
>>>>>>> e399290967834de5feafd5306ad45d86102dc7f5
...
<<<<<<< HEAD
TopicMap <- leaflet(tweet_and_topic_geo) %>% # Interactive map to see resulting clusters
=======
         text, text,
         user = user,
         u_profile = u_profile,
...
markerCol <- colorFactor(palette = (diverge_hsv(14)), tweet_and_topic_geo$topic)
...
htmlwidgets::saveWidget(TopicalMap, file = "MapTopicsFloodCluster.html")
