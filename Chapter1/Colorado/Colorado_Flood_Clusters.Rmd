...
keywords <- c("boulder", "boulderflood", "cowx", "$", "amp", "|", "rt")
...
k_result_flood_cluster %>%
  select(K, exclusivity, semantic_coherence) %>%
  filter(K %in% c(14,20,28)) %>% # This values should be the same used when creating the model in the R script
  unnest() %>%
  mutate(K = as.factor(K)) %>%
  ggplot(aes(semantic_coherence, exclusivity, color = K)) +
  geom_point(size = 2, alpha = 0.7) +
...
tweet_and_topic_geo$topic <- as.factor(tweet_and_topic_geo$topic) #Clusters as factors for coloring
pal <- rev(colorspace::diverge_hcl(14))
