...
colo_pre_CO <- colo_clean %>% 
  filter(stage == "preflood") 

colo_ima_CO <- colo_clean %>% 
  filter(stage == "immediate_aftermath") 

colo_post_CO <- colo_clean %>% 
  filter(stage == "postflood") 
...
After the spatial filtering only tweets sent during the Flood stage only **17%** of the total tweets were retained. **660 tweets** from the 3858 total.
...
keywords <- c("boulder", "boulderflood", "cowx") #,"cowx", "$", "amp", "|", "rt", "coflood", "flood"Maybe?
...
k_result_flood_cluster <- readRDS(file = "Data/k_result_flood_clusters.rds") # This file is in the UCSB box folder since it's too heavy for github
...
k_result_flood_cluster %>%
  ...
  filter(K %in% c(6,9,11)) %>% # This values should be the same used when creating the model in the R script
  unnest() %>%
...
```{r mapping_topics, echo=FALSE, message=FALSE, fig.width=15, fig.height=10}
ggplot() + 
  geom_sf(data = tweet_and_topic_geo) +
  facet_wrap(~topic)
```
