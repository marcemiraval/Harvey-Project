...
sandy=read.csv("Sandy_Clean.csv")
...
sandy <- select(sandy, -starts_with("created_at")) #removing text column

sandymap <-ggplot(sandy, aes(x=longitude, y=latitude, frame=date_time, cumulative = TRUE)) +
  borders("state",fill="#252525",size=0.1)+
  geom_point(color = "cyan", size=0.2)+
...
gganimate(sandymap,ani.width=1200, ani.height=800,interval=0.15,"sandy_cum.gif")  
...
east_coast<-ggplot(sandy_east, aes(x=longitude, y=latitude, frame=date))+
...
gganimate(east_coast,ani.width=1200, ani.height=800,interval=0.15,"east coast_date.gif")  
