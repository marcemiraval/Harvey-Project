...
for (i in nHex){
  sf_hex <- HexPoints2SpatialPolygons(spsample(sandy_boundary_sp, n = i, type = "hexagonal")) %>% 
    st_as_sf(sp_hex)
  hexagons[[i]] <- sf_hex # Creates a Large list (2000 elements, 5.4 Mb)
}  

# ggplot() + geom_sf(data = hexagons[[1000]]) # Testing results
...
#plot(hexagons[[1]])
ggplot() + geom_sf(data = hexagons[[1]]) # Not sure why plot works here and not ggplot
...
#sapply(hexagons, plotHex)

### Manana check sapply with a function of two arguments. Also first check computing totals outside function.
computeTotals <- function(hex_dfs) {
  hexWithSpoReports <- st_join(hex_dfs, sandySpotIn_sf) %>%
    group_by(group) %>%
...
Totals <- lapply(hexagons, computeTotals)

bins <- c(0, 1, 2, 3, 4, 5, 6, 7)
pal <- colorBin("YlOrRd", domain = Totals[[1]]$total, bins = bins)

SandyHexSpotMap <- leaflet(Totals[[1]]) %>%
  setView(-74.9, 40.4, 6) %>%
