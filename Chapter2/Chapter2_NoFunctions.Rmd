...
sp_hex1 <- HexPoints2SpatialPolygons(spsample(sandy_boundary_sp, 
                                              type = "hexagonal", 
                                              cellsize = 0.1))

sp_hex2 <- HexPoints2SpatialPolygons(spsample(sandy_boundary_sp, 
                                              type = "hexagonal", 
                                              cellsize = 0.3, 
                                              pretty = TRUE))
...
sf_hex1 <- st_as_sf(sp_hex1) %>% 
  mutate(group = 1:nrow(.))

sf_hex2 <- st_as_sf(sp_hex2) %>% 
  mutate(group = 1:nrow(.))


ggplot() + geom_sf(data = sf_hex)
ggplot() + geom_sf(data = sf_hex1)
ggplot() + geom_sf(data = sf_hex2)
```

```{r}
par(mfrow = c(2, 3), mar = c(0, 0, 2, 0))
for (i in 1:6) {
  new_cells <- calculate_grid(shape = sandy_boundary_sp, grid_type = "hexagonal", seed = i)
  plot(new_cells, main = paste("Seed", i, sep = " "))
}
calculate_grid(shape =sandyTweetsIn_sf, grid_type = "hexagonal", seed = 1)
```
