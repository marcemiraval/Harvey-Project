...
```{r create_hexagonal_grid, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r create_hexagonal_grid_points, include=FALSE, echo=FALSE}
...
sandy_boundary_sp <- as(sandyTweetsIn_sf, 'Spatial')
sp_hex <- HexPoints2SpatialPolygons(spsample(sandy_boundary_sp,
                                                   n = 800, 
                                                   type="hexagonal"))
sf_hex <- st_as_sf(sp_hex) %>% 
...
```{r create_hists_NWS, warning= FALSE, echo=FALSE, message= FALSE}
plotStotal <- ggplot(hexWithSpoReports, aes(total)) + geom_histogram()
plotSratio <- ggplot(hexWithSpoReports, aes(totRatio)) + geom_histogram()
grid.arrange(plotStotal, plotSratio, ncol = 2, top = "NWS reports Distribution")
```
...
binsTweets <- c(0, 0.00096, 0.00327, 0.00682, 0.01647, 0.04823, 0.3200)  # Breaks according to Natural Breaks (Jenks) classification.
...
plotTtotal <- ggplot(hexWithTweets, aes(total)) + geom_histogram()
plotTratio <- ggplot(hexWithTweets, aes(totRatio)) + geom_histogram()
grid.arrange(plotTtotal, plotTratio, ncol = 2, top = "Tweets Distribution")
```
