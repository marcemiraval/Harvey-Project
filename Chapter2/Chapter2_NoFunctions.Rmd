...
```{r create_hexagonal_boundary_grid, include=FALSE, echo=FALSE}
# 800 hexagons covering area of study.
...
```{r mapHexSandy_spotters_manual, warning= FALSE, echo=FALSE, message= FALSE, eval= FALSE}
bins <- c(0, 1, 2, 3, 4, 5, 6, 8)
pal <- colorBin("YlOrRd", domain = hexWithSpoReports$total, bins = bins)

SandyHexSpotMapMan <- leaflet(hexWithSpoReports) %>%
  setView(-74.9, 40.4, 6) %>%
  addProviderTiles(providers$OpenStreetMap.BlackAndWhite) %>% 
...
```{r mapHexSandy_spotters_Norm_manual, warning= FALSE, echo=FALSE, message= FALSE, eval= FALSE}
...
SandyHexSpotNorMapMan <- leaflet(hexWithSpoReports) %>%
  setView(-74.9, 40.4, 6) %>%
  addProviderTiles(providers$OpenStreetMap.BlackAndWhite) %>% 
...
```{r mapHexSandy_Tweets_manual, warning= FALSE, echo=FALSE, message= FALSE, eval= FALSE}
binsTweets <- c(0, 1, 20, 100, 170, 322, 14500, 24000)
palTweets <- colorBin("YlOrRd", domain = hexWithTweets$total, bins = binsTweets)

SandyHexTweetMapMan <- leaflet(hexWithTweets) %>%
  setView(-74.9, 40.4, 6) %>%
  addProviderTiles(providers$OpenStreetMap.BlackAndWhite) %>% 
  addPolygons(fillColor = ~palTweets(total),
              ...
              dashArray = "2",
              fillOpacity = 0.7,
              popup = ~htmlEscape(sprintf("Tweets per hexagon: %i", total))) %>%
...
```{r mapHexSandyNormTweets_manual, warning= FALSE, echo=FALSE, message= FALSE, eval= FALSE}

binsTweets <- c(0, 0.00096, 0.00327, 0.00682, 0.01647, 0.04823, 0.3250)  # Breaks according to Natural Breaks (Jenks) classification.

palTweets <- colorBin("YlOrRd", domain = hexWithTweets$totRatio, bins = binsTweets)

SandyHexTweetNorMapMan <- leaflet(hexWithTweets) %>%
  setView(-74.9, 40.4, 6) %>%
  addProviderTiles(providers$OpenStreetMap.BlackAndWhite) %>% 
  addPolygons(fillColor = ~palTweets(totRatio),
              ...
              dashArray = "2",
              fillOpacity = 0.7,
              popup = ~htmlEscape(sprintf("Tweets per hexagon: %f", 
                                          totRatio))) %>% 
  addLegend(pal = palTweets, 
            values = ~totRatio, 
            opacity = 0.7, 
